...
st.set_page_config(page_title="Auto Crop + LaTeX/Word (Cloud Ready)", layout="wide")
st.title("📄 Auto-crop minh họa (Gemini Flash) + OCR AI → LaTeX/Word (GPT-4o AI.VN)")

# Nhập key ở sidebar cho bảo mật
...
GPT4O_ENDPOINT = "https://api.sv2.llm.ai.vn/v1/chat/completions"
...
def gpt4o_ocr_format(image, api_key, mode="latex", n_img=0):
    ...
    try:
        resp = requests.post(GPT4O_ENDPOINT, headers=headers, json=payload, timeout=180)
        data = resp.json()
...
st.info("Ảnh sẽ auto crop minh họa bằng Gemini Flash. Nếu không đúng, crop tay. Nội dung OCR & định dạng LaTeX/Word bằng GPT-4o.")
...
if uploaded and (g_api_key and api_key):
    ...
    for idx, img in enumerate(images):
        ...
        boxes = []
        if g_api_key:
            with st.spinner("Gemini Flash đang nhận diện vùng hình minh họa..."):
                boxes = gemini_detect_boxes(img, g_api_key)
        crops_this_page = []
        if boxes:
            st.success(f"Gemini Flash phát hiện {len(boxes)} vùng hình minh họa.")
            for i, (left, top, width, height) in enumerate(boxes):
                crop = img.crop((left, top, left + width, top + height))
                st.image(crop, caption=f"Minh họa auto {i+1} Trang {idx+1}", use_container_width=True)
                crops_this_page.append(crop)
        else:
            st.warning("Gemini KHÔNG phát hiện minh họa. Bạn crop tay:")
            n_crop = st.number_input(f"Số vùng minh họa muốn crop tay (Trang {idx+1})", 1, 10, 1, 1)
        ...
        if st.button(f"OCR & chuyển định dạng (GPT-4o) Trang {idx+1}", key=f"ocr_{idx}"):
            with st.spinner("GPT-4o AI.VN đang nhận diện & định dạng..."):
    ...
    st.info("Nhập đủ 2 API key và tải lên file để bắt đầu.")

st.caption("✨ Auto crop bằng Gemini Flash. Crop tay nếu cần. OCR + định dạng bằng GPT-4o AI.VN.")
